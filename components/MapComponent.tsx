...
export default function MapComponent({
  ...
  const layersRef = useRef<{
    ...
    pickedLocation?: L.Marker | L.LayerGroup;
  }>({});
  ...
  useEffect(() => {
    ...
    if (!isPickerActive || !onLocationPicked) return;
  ...
  }, [isPickerActive, onLocationPicked]);
  ...
  useEffect(() => {
        ...
        kelurahanGeojson as Parameters<typeof L.geoJSON>[0],
        {
          style: (feature) => {
            ...
            if (isKelurahan) {
              ...
              const fillColor =
                kelurahanColors[Math.abs(hash) % kelurahanColors.length];
              return {
          ...
          onEachFeature: (feature, layer) => {
            ...
            if (props.tipe === "kelurahan") {
              layer.on("click", () => {
                ...
                onSelectKelurahan?.(
                  selectedKelurahan === props.nama ? null : props.nama,
                );
              });
            } else {
              layer.bindPopup(`
    ...
    if (showJalan && sekolahGeojson) {
      const allRoutesFeatures = sekolahGeojson.features
        ...
        .map((f) => ({
          ...
          properties: {
            sekolah_id: f.properties.id,
            tipe: "distribusi_subtle",
          },
        }));
    ...
    if (selectedSekolahId && sekolahGeojson) {
      const sel = sekolahGeojson.features.find(
      ...
      layersRef.current.sekolah = L.geoJSON(
        sekolahGeojson as Parameters<typeof L.geoJSON>[0],
        {
          pointToLayer: (feature, latlng) => {
            ...
            const isUnderSelectedSppg = !!(
              selectedSppgId && props.id_sppg === selectedSppgId
            );
            ...
            if (selectedRekomendasiId && isBlankSpot && rekomendasiValidasi) {
              isUnderSelectedRekomendasi = rekomendasiValidasi.some(
                (v) =>
                  v.nama_sekolah === props.nama &&
                  String(v.kluster_id) === selectedRekomendasiId,
              );
            }
          ...
          onEachFeature: (feature, layer) => {
            const props = feature.properties as SekolahProperties;
            const isBlankSpot = !props.id_sppg;
            const servingSppg = sppgGeojson?.features.find(
              (f) => f.properties.id === props.id_sppg,
            );
        ...
        rekomendasiGeojson as Parameters<typeof L.geoJSON>[0],
        {
          ...
          `;
            ...
            if (onSelectRekomendasi) {
              marker.on("click", () => {
                if (isPickerActive) return;
                onSelectRekomendasi(
                  isSelected ? null : String(props.kluster_id),
                );
              });
            }
          ...
          onEachFeature: (feature, layer) => {
            ...
            <div class="p-1 text-[#1C322D] font-sans">
              ...
              <p class="text-[10px] text-slate-400 mt-2">Dihitung otomatis via ST_Centroid basis radius jangkauan.</p>
            </div>
          `);
          },
        },
    ...
    if (pickedLocation) {
      ...
      const markerHtml = isSekolah
        ? `
          <div class="relative flex items-center justify-center w-7 h-7 rounded-full border border-[#1C322D] shadow-md scale-110 bg-[#EC4899] text-white ring-4 ring-[#EC4899]/40">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="m4 6 8-4 8 4" />
              <path d="m18 10 4 2v8a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-8l4-2" />
              <path d="M14 22v-4a2 2 0 1 0-4 0v4" />
            ...
            <span class="absolute -inset-1 rounded-full border-2 border-[#EC4899]/60 animate-ping opacity-70"></span>
          </div>
        `
        : `
          <div class="relative flex items-center justify-center w-9 h-9 rounded-xl border border-[#1C322D] shadow-lg shadow-[#1C322D]/35 scale-110 bg-[#EC4899] text-white ring-4 ring-[#EC4899]/40">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" />
              <polyline points="9 22 9 12 15 12 15 22" />
            </svg>
      ...
      const customIcon = L.divIcon({
        html: markerHtml,
        className: isSekolah ? "custom-school-icon" : "custom-sppg-icon",
        iconSize: isSekolah ? [28, 28] : [36, 36],
        iconAnchor: isSekolah ? [14, 14] : [18, 18],
      });
      ...
      marker.bindPopup(`
        <div class="p-1 text-[#1C322D] font-sans">
          <span class="px-2 py-0.5 text-[10px] font-bold rounded-full bg-[#EC4899]/20 text-[#EC4899]">Draft Lokasi Baru</span>
          <h3 class="font-bold text-sm text-slate-900 mt-2">Menambahkan ${isSekolah ? "Sekolah" : "SPPG"}</h3>
          <p class="text-[10px] text-slate-500 mt-1">Lat: ${pickedLocation.lat.toFixed(6)}</p>
          <p class="text-[10px] text-slate-500">Lng: ${pickedLocation.lng.toFixed(6)}</p>
        </div>
      `);
  ...
  return (
    <div
      ref={mapContainerRef}
      className="w-full h-full bg-[#F8F3EE] rounded-2xl overflow-hidden"
    />
  );
}
